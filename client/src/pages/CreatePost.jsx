    ...
    const generateImage = async() => {
      if(form.prompt) {
        try {
          ...
          const data = await response.json(); //from dalleRoute.js we are getting this -> res.status(200).json({photo:image})
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
           ...
           <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500
            w-64 h-64 flex justify-center items-center">
             {form.photo ? (
                <img
                src={form.photo}
                alt={form.prompt} 
        ...
        <div className="mt-10">
          ...
          <button
           type="submit"
           className="text-white bg-[#6469ff] font-medium rounded-md text-sm w-full sm:w-auto px-3 py-2 text-center mt-3"
          >
